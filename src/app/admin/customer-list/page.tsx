...
const CustomerList = () => {
  ...
  const renderPagination = () => {
    const pageNumbers = [];
    const maxPagesToShow = 5; // Max page numbers to show for responsiveness
    ...
    <div className="bg-white shadow-md rounded-lg dark:bg-gray-800 p-4 sm:p-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            <div className="relative w-full sm:w-auto md:w-64">
              ...
              <Input
                type="text" placeholder="Search owners..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
        ...
        <div className="absolute right-4 left-4 mt-2 sm:left-auto sm:w-72 bg-white rounded-lg shadow-xl border border-gray-200 p-4 z-10 dark:bg-gray-900 dark:border-gray-700">
          <div className="space-y-4">
            ...
            <div className="flex justify-end gap-2">
              <Button onClick={() => setFilters({ location: "", category: "", status: "" })} className="px-4 py-2 text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800 rounded-md">Clear</Button>
      ...
      <div className="mt-4">
        {loading ? (
                              ...
                              <td className="p-4 relative">
                                ...
                                {activeDropdown === customer.id && (
                                  <div className="absolute right-0 mt-2 w-40 bg-white rounded-md shadow-lg border border-gray-200 z-10 dark:bg-gray-800 dark:border-gray-700">
                                    <Button onClick={() => handleViewProfile(customer.id)} className="w-full px-4 py-2 text-sm text-left  hover:bg-gray-700 flex items-center gap-2 text-white dark:text-black dark:hover:bg-gray-700"><Eye className="w-4 h-4" /> View</Button>
                                    <Button onClick={() => handleEditCustomer(customer.id)} className="w-full px-4 py-2 text-sm text-left text-white hover:bg-gray-700 flex items-center gap-2 dark:text-black dark:hover:bg-gray-700"><Edit className="w-4 h-4" /> Edit</Button>
                                    <Button onClick={() => handleDeleteCustomer(customer.id)} className="w-full px-4 py-2 text-sm text-left text-red-600 hover:bg-red-100 flex items-center gap-2 dark:hover:bg-red-900/50"><UserMinus className="w-4 h-4" /> Delete</Button>
                                  </div>
                                )}
                              </td>
            ...
            <div className="p-4 text-center text-gray-500">No owners match your criteria.</div>
        )}
      </div>
